...
describe('players selectors', () => {
  ...
  it('should filter by age', () => {
    expect(filterByAgeSelector(state)).toEqual(state.playersReducer.players.filter(player => player.age === state.playersReducer.searchTerms.playerAge));
  });

  it('should filter by name', () => {
    expect(filterByNameSelector(state)).toEqual(state.playersReducer.players.filter(player => player.name.includes(state.playersReducer.searchTerms.playerName)));
  });

  it('should filter by position', () => {
    expect(filterByPositionSelector(state)).toEqual(state.playersReducer.players.filter(player => player.position === state.playersReducer.searchTerms.playerPosition));
  });
  ...
  it('empty age field', () => {
    expect(filterByAgeSelector(emptySearchTermsState)).toEqual(playersSelector(state));
  });

  it('empty name field', () => {
    expect(filterByNameSelector(emptySearchTermsState)).toEqual(playersSelector(state));
  });

  it('empty position field', () => {
    expect(filterByPositionSelector(emptySearchTermsState)).toEqual(playersSelector(state));
  }); 
});